...
export class FilesFieldsInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler) {
    const multerMiddleware = multer({
      storage: diskStorage({
        destination: (req, file, callback) => {
          const dest = this.fields.find(
            (fi) => fi.field === file.fieldname,
          ).dest
          callback(null, dest)
        },
        ...
        filename: (req, file, callback) => {
          ...
          const fileNameFn = this.fields.find(
            (fi) => fi.field === file.fieldname,
          )?.editFileName
          const fileName = fileNameFn
            ? fileNameFn(req, file, callback)
            : editFileName(req, file, callback)
          this.fileSystemService.pushUploadedFile(fileName)
        },
      }),
      fileFilter: (req, file, callback) => {
        const fileFilter = this.fields.find(
          (fi) => fi.field === file.fieldname,
        ).fileFilter
        return fileFilter(req, file, callback)
      },
    }).fields(
      this.fields.map((f) => ({ name: f.field, maxCount: f.maxCount ?? 1 })),
    )
    ...
    return new Promise((resolve, reject) =>
      multerMiddleware(request, undefined, (error: any) =>
